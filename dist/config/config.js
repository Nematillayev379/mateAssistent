...
exports.CONFIG = {
    TELEGRAM_TOKEN: (() => {
        ...
        return secret_manager_1.SecretManager.get('TELEGRAM_TOKEN') || process.env.TELEGRAM_BOT_TOKEN || '';
    })(),
    MAX_TOKENS: 2000,
    TEMPERATURE: 0.6,
    ...
    SECRET_BACKEND: (process.env.SECRET_BACKEND || "env"),
...
function shouldUseDefaultRedisUrl(url) {
    if (!url.trim())
        return false;
...
function collectProviderKeys(envPrefix, type, singleEnvNames) {
    const found = [];
    const bulk = secret_manager_1.SecretManager.get(`${envPrefix}_KEYS`);
    for (const k of parseKeyList(bulk)) {
        found.push({ key: k, type });
    }
    for (const name of singleEnvNames) {
        const v = secret_manager_1.SecretManager.get(name);
        if (!v?.trim())
            continue;
    ...
    const allSecrets = secret_manager_1.SecretManager.getAllMatching(prefixUpper);
...
function getEnvKeySourceReport() {
    return secret_manager_1.SecretManager.getKeyReport();
}
