...
if (!connectionOptions) {
    ...
    const aiWorker = new bullmq_1.Worker("ai-queue", async (job) => {
        ...
        try {
            ...
            await (0, sender_1.safeSend)(user, enrichedArticle);
    ...
    aiWorker.on("error", (err) => {
        if (err.message.includes("limit exceeded") || err.message.toLowerCase().includes("exceeded")) {
            logger_1.logger.warn(`AI worker: limit exceeded (pool rotating automatically)`);
        }
        else {
            logger_1.logger.error(`AI worker error: ${err.message}`);
        }
    });
    logger_1.logger.info("AI Worker started with Redis pool");
}
