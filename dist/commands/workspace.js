...
exports.workspaceCommand = {
    ...
    handler: async (bot, msg, match) => {
        ...
        const workspaces = await database_1.DBService.getUserWorkspaces(chatId);
        if (!raw) {
            const list = workspaces.length > 0
                ? workspaces.map((ws, i) => `${i + 1}. <b>${ws.name}</b> — ${ws.channel_count || 0} kanal\n` +
                    `<code>/workspace switch ${ws.id}</code>`).join('\n')
        ...
        if (sub === 'switch' && parts[1]) {
            ...
            await database_1.DBService.setSetting(`active_ws_${chatId}`, String(wsId));
            return bot.sendMessage(chatId, `✅ Aktiv workspace: <b>${targetWs.name}</b>\n\nEndi barcha kanallarga post yuboriladi.`, { parse_mode: 'HTML' });
        }
        if (sub === 'off' || sub === 'none') {
            await database_1.DBService.setSetting(`active_ws_${chatId}`, '');
        ...
        if (sub === 'channels' && parts[1]) {
            ...
            if (!ws)
                return bot.sendMessage(chatId, 'Workspace topilmadi.', { parse_mode: 'HTML' });
            const channels = await database_1.DBService.getWorkspaceChannels(wsId);
            const list = channels.length > 0
                ? channels.map((ch) => `— ${ch.name} (${ch.channel_id})`).join('\n')
