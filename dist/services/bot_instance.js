...
const dashboardTokenSecret = config_1.CONFIG.DASHBOARD_SECRET?.trim() || crypto_1.default.randomBytes(32).toString('hex');
if (!config_1.CONFIG.DASHBOARD_SECRET?.trim()) {
    logger_1.logger.warn('DASHBOARD_SECRET is missing; using an ephemeral secret for per-user dashboard tokens.');
}
/**
 * Bug #27 Fix: Generates a unique token for each user to prevent IDOR
 */
function generateDashboardToken(userId) {
    return crypto_1.default.createHash('sha256').update(`${userId}:${dashboardTokenSecret}`).digest('hex').slice(0, 32);
}
