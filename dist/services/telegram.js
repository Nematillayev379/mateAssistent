...
async function startBot() {
    ...
    if (config_1.CONFIG.PUBLIC_URL && process.env.NODE_ENV !== "development") {
        try {
            ...
            const whInfo = await bot_instance_1.bot.getWebHookInfo();
            ...
            if (err.message.includes('409') || err.message.includes('Conflict')) {
                logger_1.logger.warn('Webhook conflict — not falling back to polling');
            }
            else {
        ...
        await bot_instance_1.bot.deleteWebHook().catch((delErr) => logger_1.logger.warn(`Webhook delete error: ${delErr.message}`));
        initPolling();
    ...
    if (config_1.CONFIG.OWNER_ID != null) {
        try {
            ...
            logger_1.logger.warn(`Owner notify failed: ${e.message}`);
        }
    }
}
function initPolling() {
    ...
    if (!pollingErrorHandlerAttached) {
        bot_instance_1.bot.on("polling_error", (error) => { handlePollingError(error); });
...
function startPollingSafe() {
    ...
    try {
        ...
        Promise.resolve(maybePromise)
            ...
            .catch((error) => { logger_1.logger.error(`startPolling error: ${getPollingErrorMessage(error)}`); });
    }
    catch (error) {
