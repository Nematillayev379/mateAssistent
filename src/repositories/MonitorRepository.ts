...
export const MonitorRepository = {
  async getByUser(userId: number) {
    ...
    if (error) console.error(`getUserMonitoredChannels error: ${error.message}`);
    return data || [];
  },
  ...
  async add(userId: number, platform: string, channelId: string, name: string, opts?: { forward_mode?: string; use_ai?: number }) {
    ...
    if (error) console.error(`addMonitoredChannel error: ${error.message}`);
  },
  ...
  async updateSettings(id: number, userId: number, updates: Record<string, any>) {
    ...
    if (error) console.error(`updateMonitoredChannelSettings error: ${error.message}`);
  },

  async remove(userId: number, id: number) {
    const { error } = await getSupabase().from('monitored_channels').delete().eq('id', id).eq('user_id', userId);
    if (error) console.error(`removeMonitoredChannel error: ${error.message}`);
  },

  async getAll() {
    const { data, error } = await getSupabase().from('monitored_channels').select('*');
    if (error) console.error(`getMonitoredChannels error: ${error.message}`);
    return data || [];
  },
  ...
  async updateLastPost(id: number, lastPostId: string) {
    ...
    if (error) console.error(`updateMonitoredChannel error: ${error.message}`);
  },
};
...
export const TelegramMessageRepository = {
  ...
  async markSeen(userId: number, sourceChatId: string, messageId: number) {
    ...
    if (error && !String(error.message).includes('duplicate') && error.code !== '23505') console.warn(`markTelegramMessageSeen: ${error.message}`);
  },
};
