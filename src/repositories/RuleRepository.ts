...
export const RuleRepository = {
  async getByUser(userId: number): Promise<any[]> {
    ...
    if (error) { console.error(`getUserRules error: ${error.message}`); return []; }
    return data || [];
  },
  ...
  async add(userId: number, trigger: string, condition: string, action: string, actionValue: string): Promise<boolean> {
    ...
    if (error) { console.error(`addRule error: ${error.message}`); return false; }
    return true;
  },
  ...
  async toggle(ruleId: number, isActive: boolean): Promise<boolean> {
    ...
    if (error) { console.error(`toggleRule error: ${error.message}`); return false; }
    return true;
  },

  async delete(ruleId: number): Promise<boolean> {
    const { error } = await getSupabase().from('automation_rules').delete().eq('id', ruleId);
    if (error) { console.error(`deleteRule error: ${error.message}`); return false; }
    return true;
  },
};
...
export const TicketRepository = {
  ...
  async getByUser(userId: number) {
    ...
    if (error) console.error(`getUserTickets error: ${error.message}`);
    return data || [];
  },
  ...
  async getAll() {
    ...
    if (error) console.error(`getTickets error: ${error.message}`);
    return data || [];
  },
...
export const DraftRepository = {
  async save(userId: number, draft: { title?: string; body: string; image_url?: string; channels?: string[] }) {
    ...
    if (error) console.error(`savePostDraft error: ${error.message}`);
    return data;
  },
