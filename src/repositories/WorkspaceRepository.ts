...
export const WorkspaceRepository = {
  async getByUser(userId: number): Promise<any[]> {
    ...
    if (error) { console.error(`getUserWorkspaces error: ${error.message}`); return []; }
    return data || [];
  },
  ...
  async create(userId: number, name: string): Promise<any> {
    ...
    if (error) { console.error(`createWorkspace error: ${error.message}`); return null; }
    return data;
  },
  ...
  async getChannels(workspaceId: number): Promise<any[]> {
    ...
    if (error) { console.error(`getWorkspaceChannels error: ${error.message}`); return []; }
    return data || [];
  },
  ...
  async addChannel(workspaceId: number, channelId: string, name: string): Promise<boolean> {
    ...
    if (error) { console.error(`addWorkspaceChannel error: ${error.message}`); return false; }
    return true;
  },
  ...
  async removeChannel(channelId: string, workspaceId: number): Promise<boolean> {
    ...
    if (error) { console.error(`removeWorkspaceChannel error: ${error.message}`); return false; }
    return true;
  },
};
